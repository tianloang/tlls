...
public class CommonDateUtils {
    ...
    public enum passportVisaType{
    	 ...
    	 * 签证、港澳通行证、台湾证提前5天结束
    	 */
    	VISA_HONGKONGMACAUEEP_TaiWanEEP
    }
    ...
    public static Date getDateByTime(Date date, String time) {
        ...
        calendar.setTime(date);
        calendar.set(calendar.get(Calendar.YEAR),calendar.get(Calendar.MONTH),calendar.get(Calendar.DAY_OF_MONTH),departHour,departMin,00);
     ...
     * @param endDate   到期时间
     * @param type   到期时间  护照和大陆证提前190结束，签证、港澳通行证、台湾证提前5天结束
     * @return
     */
    public static Map<String,String> getPassVisaAvailableDate(Date enableDate,Date closeDate,Date endDate,String type){
    	Map<String,String> map = new HashMap<>();
    	if(type != null && enableDate != null) {
    		String startDate = dateToString(enableDate);
    		Date realEndDate = (closeDate == null ? endDate : closeDate);
    		if(realEndDate == null) return null;
    		//计算失效日期
			Instant instant = realEndDate.toInstant();
		    ZoneId zone = ZoneId.systemDefault();
		    LocalDateTime localDateTime = LocalDateTime.ofInstant(instant, zone);
		    LocalDate localDate = localDateTime.toLocalDate();
		   //签证有限时间
		    String endDateStr = "";
		    if(type.equals(passportVisaType.PASSPORT_CHINA.name())) {
		    	endDateStr = localDate.minusDays(189).format(DateTimeFormatter.ofPattern("yyyy-MM-dd"));
	    	}else if(type.equals(passportVisaType.VISA_HONGKONGMACAUEEP_TaiWanEEP.name())){
	    		endDateStr = localDate.minusDays(4).format(DateTimeFormatter.ofPattern("yyyy-MM-dd"));
	    	}
